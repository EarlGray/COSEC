...
uint32_t
digital_update_crc32(uint32_t crc, const uint8_t *data, size_t len)
{
    ...
    {
      uint8_t b = *data++;
      b = (nib_reverse[b&0x0f] << 4) | (nib_reverse[b>>4]);
...
uint32_t ethernet_crc32(uint8_t *b, size_t n) {
    uint32_t crc = digital_crc32(b, n);
    uint32_t rcrc;
